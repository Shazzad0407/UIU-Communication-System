...
// finding which which courses selected
    $q ="WHERE";
    //$i=0;
    for($i=0; $i<$count-1; $i++){
        $p= " courseId = '$catG[$i]' OR";
        $q = $q.$p;
    }
    $p= " courseId = '$catG[$i]'";
    $q = $q.$p;
...
$sq= "SELECT*
      FROM ( SELECT*
            FROM (SELECT DISTINCT post.postId,post.studentId,post.description,post.postTime,course.courseId,course.courseName
                  FROM post INNER JOIN course ON (post.courseId = course.courseId)) as c
                  $q) AS b NATURAL JOIN student
